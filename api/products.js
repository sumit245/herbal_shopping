...
module.exports.uploadImage = async (req, res) => {
    try {
        ...
        const product = await Product.findByIdAndUpdate(id, { $set: { image_url: `${req.file.destination}/${req.file.filename}` } })
        if (!product) {
...
module.exports.getProduct = async (req, res) => {
    try {
        ...
        if (data.length < 1) {
            return res.status(404).send({ message: "Product Not found!!!", count: 0 })
        }
        res.status(200).send({ message: "Product Fetched  successfully", data: data, count: data.length })
    } catch (err) {
        res.status(500).send(err)
    }
}
